...
public class DashboardAdminClient {
	...
	public static DashboardAdminServiceStub getDashboardAdminService(ServletConfig config,
	                                                                 HttpSession session,
		...
		String serverURL = CarbonUIUtil.getServerURL(config.getServletContext(), session) +
		                   "DashboardAdminService.DashboardAdminServiceHttpsSoap12Endpoint";
	...
	public static DataViewDTO toDataViewDTO(DataView dataView) {
		DataViewDTO dto =
				new DataViewDTO(dataView.getId(), dataView.getDisplayName(), dataView.getType());
		...
		if (dataView.getWidgets() != null) {
			...
			for (Widget widget : dataView.getWidgets()) {
				widgetDTOs
						.add(new WidgetDTO(widget.getId(), widget.getTitle(), widget.getConfig()));
			}
