...
public class EventStreamUIUtils {
	...
	public static EventStreamAdminServiceStub getEventStreamAdminService(ServletConfig config,
	                                                                     HttpSession session,
		...
		EventStreamAdminServiceStub stub =
				new EventStreamAdminServiceStub(configContext, serverURL);

		String cookie = (String) session
				.getAttribute(org.wso2.carbon.utils.ServerConstants.ADMIN_SERVICE_COOKIE);
	...
	public static EventStreamDefinitionDto getEventStreamInfoDtoFrom(String jsonStreamDef) {
		...
		for (JsonElement metaAttribute : metaData) {
			...
			eventStreamAttributeDto
					.setAttributeName(metaAttributeObj.getAsJsonPrimitive("name").getAsString());
			eventStreamAttributeDto
					.setAttributeType(metaAttributeObj.getAsJsonPrimitive("type").getAsString());
			metaAttributeDtos.add(eventStreamAttributeDto);
		}
		List<EventStreamAttributeDto> correlationAttributeDtos =
				new ArrayList<EventStreamAttributeDto>();
		...
		List<EventStreamAttributeDto> payloadAttributeDtos =
				new ArrayList<EventStreamAttributeDto>();
		...
		if (metaAttributeDtos.size() > 0) {
			eventStreamInfoDto.setMetaData(metaAttributeDtos.toArray(
					new EventStreamAttributeDto[metaAttributeDtos.size()]));
		}
		if (correlationAttributeDtos.size() > 0) {
			eventStreamInfoDto.setCorrelationData(correlationAttributeDtos.toArray(
					new EventStreamAttributeDto[correlationAttributeDtos.size()]));
		}
		if (payloadAttributeDtos.size() > 0) {
			eventStreamInfoDto.setPayloadData(payloadAttributeDtos.toArray(
					new EventStreamAttributeDto[payloadAttributeDtos.size()]));
		}
