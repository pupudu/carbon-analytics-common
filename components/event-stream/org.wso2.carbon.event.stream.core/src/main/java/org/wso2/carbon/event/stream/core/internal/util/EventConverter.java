...
public class EventConverter {
	...
	public static Event convertToWso2Event(Object[] objArray, StreamDefinition streamDefinition) {
		...
		if (streamDefinition.getPayloadData() !=
		    null) { // If there is at least 1 payload data field
		...
		return new Event(streamDefinition.getStreamId(), System.currentTimeMillis(), metaAttributes,
		                 correlationAttributes, payloadAttributes);
	}

	public static Object[] convertToEventData(Event event, boolean metaFlag,
	                                          boolean correlationFlag, boolean payloadFlag,
	                                          int size) {
