...
public class SampleEventGenerator {
	...
	public static String generateXMLEvent(StreamDefinition streamDefinition)
		...
		OMElement sampleEventsElement =
				factory.createOMElement(new QName(EventStreamConstants.SAMPLE_EVENTS_PARENT_TAG));

		OMElement sampleEventElement =
				factory.createOMElement(new QName(EventStreamConstants.SAMPLE_EVENT_PARENT_TAG));
		...
		if (metaDatAttributes != null && metaDatAttributes.size() > 0) {
			sampleEventElement.addChild(createPropertyElement(factory, metaDatAttributes,
			                                                  EventStreamConstants.SAMPLE_EVENT_META_TAG));
		}
		...
		if (correlationAttributes != null && correlationAttributes.size() > 0) {
			sampleEventElement.addChild(createPropertyElement(factory, correlationAttributes,
			                                                  EventStreamConstants.SAMPLE_EVENT_CORRELATION_TAG));
		}

		List<Attribute> payloadAttributes = streamDefinition.getPayloadData();
		if (payloadAttributes != null && payloadAttributes.size() > 0) {
			sampleEventElement.addChild(createPropertyElement(factory, payloadAttributes,
			                                                  EventStreamConstants.SAMPLE_EVENT_PAYLOAD_TAG));
		}
	...
	public static String generateJSONEvent(StreamDefinition streamDefinition)
		...
		List<Attribute> metaDatAttributes = streamDefinition.getMetaData();
		if (metaDatAttributes != null && metaDatAttributes.size() > 0) {
			innerParentObject.add(EventStreamConstants.SAMPLE_EVENT_META_TAG,
			                      createPropertyElement(metaDatAttributes));
		}

		List<Attribute> correlationAttributes = streamDefinition.getCorrelationData();
		if (correlationAttributes != null && correlationAttributes.size() > 0) {
			innerParentObject.add(EventStreamConstants.SAMPLE_EVENT_CORRELATION_TAG,
			                      createPropertyElement(correlationAttributes));
		}

		List<Attribute> payloadAttributes = streamDefinition.getPayloadData();
		if (payloadAttributes != null && payloadAttributes.size() > 0) {
			innerParentObject.add(EventStreamConstants.SAMPLE_EVENT_PAYLOAD_TAG,
			                      createPropertyElement(payloadAttributes));
		}
	...
	public static String generateTextEvent(StreamDefinition streamDefinition)
		...
		if (metaDatAttributes != null && metaDatAttributes.size() > 0) {
			for (Attribute attribute : metaDatAttributes) {
				sampleEvent += "\n" + EventStreamConstants.META_PREFIX + attribute.getName() +
				               EventStreamConstants.EVENT_ATTRIBUTE_VALUE_SEPARATOR +
				               getSampleValue(attribute.getType()) +
				               EventStreamConstants.EVENT_ATTRIBUTE_SEPARATOR;
			}
		}
		...
		if (correlationAttributes != null && correlationAttributes.size() > 0) {
			for (Attribute attribute : correlationAttributes) {
				sampleEvent +=
						"\n" + EventStreamConstants.CORRELATION_PREFIX + attribute.getName() +
						EventStreamConstants.EVENT_ATTRIBUTE_VALUE_SEPARATOR +
						getSampleValue(attribute.getType()) +
						EventStreamConstants.EVENT_ATTRIBUTE_SEPARATOR;
			}
	...
	private static String getSampleValue(AttributeType attributeType) {
		...
		if (attributeType.name().toLowerCase().equals(EventStreamConstants.ATTR_TYPE_FLOAT)) {
		...
		} else if (attributeType.name().toLowerCase()
		                        .equals(EventStreamConstants.ATTR_TYPE_DOUBLE)) {
			int hostIndex = rand.nextInt(5);
			return doubleValues[hostIndex] + "";
		} else if (attributeType.name().toLowerCase()
		                        .equals(EventStreamConstants.ATTR_TYPE_INTEGER)) {
			int hostIndex = rand.nextInt(5);
			...
			return booleanValues[hostIndex] + "";
		                        ...
		                        .equals(EventStreamConstants.ATTR_TYPE_STRING)) {
			int hostIndex = rand.nextInt(5);
