...
public class EventStreamConfigurationFileSystemInvoker {
	private static final Log log =
			LogFactory.getLog(EventStreamConfigurationFileSystemInvoker.class);

	public static void save(StreamDefinition streamDefinition, String filePath,
	                        AxisConfiguration axisConfig) throws EventStreamConfigurationException {
		EventStreamDeployer eventStreamDeployer =
				(EventStreamDeployer) ((DeploymentEngine) axisConfig.getConfigurator()).
						                                                                       getDeployer(
								                                                                       EventStreamConstants.EVENT_STREAMS,
								                                                                       EventStreamConstants.STREAM_DEFINITION_FILE_EXTENSION_TYPE);
		try {
			...
			try {
				...
				log.info("Stream definition configuration for " + streamDefinition.getStreamId() +
			...
			throw new EventStreamConfigurationException(
					"Error while saving stream definition " + streamDefinition.getStreamId(), e);
		}
	}

	public static void delete(String fileName, AxisConfiguration axisConfig)
			throws EventStreamConfigurationException {

		try {
			String directoryPath =
					new File(axisConfig.getRepository().getPath()).getAbsolutePath() +
					File.separator + EventStreamConstants.EVENT_STREAMS;
			String filePath = directoryPath + File.separator + fileName;
			File file = new File(filePath);
			if (file.exists()) {
				EventStreamDeployer deployer =
						(EventStreamDeployer) ((DeploymentEngine) axisConfig.getConfigurator()).
								                                                                       getDeployer(
										                                                                       EventStreamConstants.EVENT_STREAMS,
										                                                                       EventStreamConstants.STREAM_DEFINITION_FILE_EXTENSION_TYPE);
			...
			throw new EventStreamConfigurationException(
					"Error while deleting the stream definition file ", e);
		}
	}
}
