...
public class EventJunction implements EventProducerCallback {
	...
	public void addConsumer(SiddhiEventConsumer consumer) {
		if (!siddhiEventConsumers.contains(consumer)) {
			log.info("Consumer added to the junction. Stream:" +
			         getStreamDefinition().getStreamId());
	...
	public void addConsumer(WSO2EventConsumer consumer) {
		if (!wso2EventConsumers.contains(consumer)) {
			log.info("WSO2EventConsumer added to the junction. Stream:" +
			         getStreamDefinition().getStreamId());
			consumer.onAddDefinition(streamDefinition);
			wso2EventConsumers.add(consumer);
		} else {
			log.error("WSO2EventConsumer already exist in the junction: " +
	...
	public void addProducer(EventProducer listener) {
		if (!producers.contains(listener)) {
			log.info("Producer added to the junction. Stream:" +
			         getStreamDefinition().getStreamId());
	...
	@Override public void sendEventData(Object[] data) {
	...
	@Override public void sendEvent(Event event) {
		...
		if (!siddhiEventConsumers.isEmpty() || !rawEventConsumers.isEmpty()) {
			Object[] eventData = EventConverter
					.convertToEventData(event, metaFlag, correlationFlag, payloadFlag,
					                    attributesCount);
	...
	@Override public void sendEvents(List<Event> events) {
	...
	@Override public void sendEvents(org.wso2.siddhi.core.event.Event[] events) {
		...
		for (org.wso2.siddhi.core.event.Event event : events) {
			...
			if (!wso2EventConsumers.isEmpty() || !wso2EventListConsumers.isEmpty()) {
				Event outEvent =
						EventConverter.convertToWso2Event(event.getData(), streamDefinition);
