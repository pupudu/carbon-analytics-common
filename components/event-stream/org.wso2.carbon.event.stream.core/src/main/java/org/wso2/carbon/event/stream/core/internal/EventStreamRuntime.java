...
public class EventStreamRuntime {
	...
	public void loadEventStream(String streamId) throws EventStreamConfigurationException {

		StreamDefinition streamDefinition =
				EventStreamServiceValueHolder.getCarbonEventStreamService().getStreamDefinition(
		...
		if (streamDefinition != null) {
			Map<String, EventJunction> eventJunctionMap =
					tenantSpecificEventJunctions.get(tenantId);
	...
	public void unloadEventStream(String streamId) throws EventStreamConfigurationException {
		int tenantId = PrivilegedCarbonContext.getThreadLocalCarbonContext().getTenantId();
		StreamDefinition streamDefinition =
				EventStreamServiceValueHolder.getCarbonEventStreamService().getStreamDefinition(
						DataBridgeCommonsUtils.getStreamNameFromStreamId(streamId),
	...
	private EventJunction getOrConstructEventJunction(String streamId)
		...
		if (eventJunction == null) {
			...
			try {
				streamDefinition = EventStreamServiceValueHolder.getCarbonEventStreamService()
				                                                .getStreamDefinition(streamId);
			} catch (Exception e) {
				throw new EventStreamConfigurationException(
						"Cannot retrieve Stream " + streamId + " for tenant " + tenantId);
			}
			if (streamDefinition == null) {
				throw new EventStreamConfigurationException(
						"Stream " + streamId + " is not configured to tenant " + tenantId);
			}
	...
	public void publish(String streamId, Event event) {
		...
		if (eventJunctionMap != null && eventJunctionMap.containsKey(streamId)) {
			...
			log.debug("Event " + event.toString() +
			          " dropped since no junction found for the streamId " + streamId);
		}
	}

	public void subscribe(SiddhiEventConsumer siddhiEventConsumer)
			throws EventStreamConfigurationException {
		EventJunction eventJunction =
				getOrConstructEventJunction(siddhiEventConsumer.getStreamId());
		eventJunction.addConsumer(siddhiEventConsumer);

	}

	public void subscribe(RawEventConsumer rawEventConsumer)
			throws EventStreamConfigurationException {
	...
	public void subscribe(WSO2EventConsumer wso2EventConsumer)
			throws EventStreamConfigurationException {
	...
	public void subscribe(WSO2EventListConsumer wso2EventListConsumer)
			throws EventStreamConfigurationException {
		EventJunction eventJunction =
				getOrConstructEventJunction(wso2EventListConsumer.getStreamId());
