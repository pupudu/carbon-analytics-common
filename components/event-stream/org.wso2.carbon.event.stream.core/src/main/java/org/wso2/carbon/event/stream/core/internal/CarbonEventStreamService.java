...
public class CarbonEventStreamService implements EventStreamService {
	...
	private Map<Integer, Map<String, EventStreamConfiguration>> tenantSpecificEventStreamConfigs =
			new ConcurrentHashMap<Integer, Map<String, EventStreamConfiguration>>();

	public void removeEventStreamConfigurationFromMap(String fileName) {
		int tenantId = PrivilegedCarbonContext.getThreadLocalCarbonContext().getTenantId();
		Map<String, EventStreamConfiguration> eventStreamConfigs =
				tenantSpecificEventStreamConfigs.get(tenantId);
		...
		if (streamId != null) {
			...
			for (EventStreamListener eventStreamListener : EventStreamServiceValueHolder
					.getEventStreamListenerList()) {
				eventStreamListener.removedEventStream(tenantId, DataBridgeCommonsUtils
						.getStreamNameFromStreamId(streamId), DataBridgeCommonsUtils
						                                       .getStreamVersionFromStreamId(
								                                       streamId));
			}
		}
	}
	 ...
	 * @return
	 */
	@Override public StreamDefinition getStreamDefinition(String name, String version)
	 ...
	 */
	@Override public StreamDefinition getStreamDefinition(String streamId)
			throws EventStreamConfigurationException {
		int tenantId = PrivilegedCarbonContext.getThreadLocalCarbonContext().getTenantId();
		Map<String, EventStreamConfiguration> eventStreamConfigs =
				tenantSpecificEventStreamConfigs.get(tenantId);
	...
	@Override public List<StreamDefinition> getAllStreamDefinitions()
			throws EventStreamConfigurationException {
		int tenantId = PrivilegedCarbonContext.getThreadLocalCarbonContext().getTenantId();
		Map<String, EventStreamConfiguration> eventStreamConfigs =
				tenantSpecificEventStreamConfigs.get(tenantId);
	...
	@Override public List<EventStreamConfiguration> getAllEventStreamConfigurations()
			throws EventStreamConfigurationException {
		int tenantId = PrivilegedCarbonContext.getThreadLocalCarbonContext().getTenantId();
		Map<String, EventStreamConfiguration> eventStreamConfigs =
				tenantSpecificEventStreamConfigs.get(tenantId);
	...
	public void addEventStreamConfig(EventStreamConfiguration eventStreamConfiguration)
		...
		for (EventStreamListener eventStreamListener : EventStreamServiceValueHolder
			...
			eventStreamListener.addedEventStream(tenantId,
			                                     eventStreamConfiguration.getStreamDefinition()
			                                                             .getName(),
			                                     eventStreamConfiguration.getStreamDefinition()
			                                                             .getVersion());
		}
	...
	@Override public void addEventStreamDefinition(StreamDefinition streamDefinition)
			throws EventStreamConfigurationException {
		int tenantId = PrivilegedCarbonContext.getThreadLocalCarbonContext().getTenantId();
		AxisConfiguration axisConfig = getAxisConfiguration();
		String directoryPath =
				new File(axisConfig.getRepository().getPath()).getAbsolutePath() + File.separator +
				EventStreamConstants.EVENT_STREAMS;
		...
		String filePath = directoryPath + File.separator +
		                  ...
		                  EventStreamConstants.STREAM_DEFINITION_FILE_DELIMITER +
		...
		if (streamDefinitionOld != null) {
			if (!(streamDefinitionOld.equals(streamDefinition))) {
				throw new StreamDefinitionAlreadyDefinedException(
						"Different stream definition with same stream id " +
						streamDefinition.getStreamId() + " already exist " +
						streamDefinitionOld.toString() + ", cannot add stream definition " +
	...
	private AxisConfiguration getAxisConfiguration() {
		...
		if (tenantId == MultitenantConstants.SUPER_TENANT_ID) {
			axisConfig = EventStreamServiceValueHolder.getConfigurationContextService()
	...
	public void removeEventStreamDefinition(String streamId)
			throws EventStreamConfigurationException {
		String name = null, version = null;
		if (streamId != null &&
		    streamId.contains(StreamdefinitionStoreConstants.STREAM_ID_SPLITTER)) {
	...
	@Override public void removeEventStreamDefinition(String streamName, String streamVersion)
			throws EventStreamConfigurationException {
		AxisConfiguration axisConfig = getAxisConfiguration();
		EventStreamConfigurationFileSystemInvoker
				.delete(streamName + EventStreamConstants.STREAM_DEFINITION_FILE_DELIMITER +
				        streamVersion + EventStreamConstants.STREAM_DEFINITION_FILE_EXTENSION,
				        axisConfig);

		log.info("Stream definition - " + streamName +
		         EventStreamConstants.STREAM_DEFINITION_DELIMITER + streamVersion +
		         " removed successfully");
	}

	@Override public List<String> getStreamIds() throws EventStreamConfigurationException {
	...
	@Override public String generateSampleEvent(String streamId, String eventType)
			throws EventStreamConfigurationException {
