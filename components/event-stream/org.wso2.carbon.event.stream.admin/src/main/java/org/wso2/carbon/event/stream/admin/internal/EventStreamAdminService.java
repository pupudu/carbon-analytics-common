...
public class EventStreamAdminService extends AbstractAdmin {
	...
	public void addEventStreamDefinitionAsDto(EventStreamDefinitionDto eventStreamDefinitionDto)
			throws AxisFault {
		if ((eventStreamDefinitionDto.getName() != null) &&
		    (!eventStreamDefinitionDto.getName().isEmpty())) {
			if ((eventStreamDefinitionDto.getVersion() != null) &&
			    (!eventStreamDefinitionDto.getVersion().isEmpty())) {
				try {
					StreamDefinition streamDefinition =
							new StreamDefinition(eventStreamDefinitionDto.getName(),
					...
					if (eventStreamDefinitionDto.getMetaData() != null) {
						...
						for (EventStreamAttributeDto eventStreamAttributeDto : metaEventStreamAttributeDtos) {
							streamDefinition.addMetaData(eventStreamAttributeDto.getAttributeName(),
					...
					if (correlationEventStreamAttributeDtos != null) {
						for (EventStreamAttributeDto eventStreamAttributeDto : correlationEventStreamAttributeDtos) {
							streamDefinition
									.addCorrelationData(eventStreamAttributeDto.getAttributeName(),
									                    EventAttributeTypeConstants.STRING_ATTRIBUTE_TYPE_MAP
											                    .get(eventStreamAttributeDto
													                         .getAttributeType()));
						}
					...
					if (payloadEventStreamAttributeDtos != null) {
						for (EventStreamAttributeDto eventStreamAttributeDto : payloadEventStreamAttributeDtos) {
							streamDefinition
									.addPayloadData(eventStreamAttributeDto.getAttributeName(),
									                EventAttributeTypeConstants.STRING_ATTRIBUTE_TYPE_MAP
											                .get(eventStreamAttributeDto
													                     .getAttributeType()));
						}
					...
					EventStreamService eventStreamService =
							EventStreamAdminServiceValueHolder.getEventStreamService();
	...
	public void addEventStreamDefinitionAsString(String streamStringDefinition) throws AxisFault {
		...
		try {
			streamDefinition =
					EventDefinitionConverterUtils.convertFromJson(streamStringDefinition);
			if ((streamDefinition.getName().equals("")) ||
			    (streamDefinition.getVersion().equals(""))) {
				throw new AxisFault("Empty inputs fields are not allowed.");
			} else if (streamDefinition.getCorrelationData() == null &&
			           streamDefinition.getMetaData() == null &&
	...
	public void editEventStreamDefinitionAsString(String streamStringDefinition, String oldStreamId)
			throws AxisFault {
		...
		try {
			...
			if ((streamDefinition.getName().equals("")) ||
				...
				EventStreamService eventStreamService =
						EventStreamAdminServiceValueHolder.getEventStreamService();
				eventStreamService.removeEventStreamDefinition(oldStreamProperties[0],
				                                               oldStreamProperties[1]);
				eventStreamService.addEventStreamDefinition(streamDefinition);
			}
		} catch (MalformedStreamDefinitionException e) {
			throw new AxisFault(e.getMessage(), e);
	...
	public void editEventStreamDefinitionAsDto(EventStreamDefinitionDto eventStreamDefinitionDto,
	                                           String oldStreamId) throws AxisFault {
	...
	public void removeEventStreamDefinition(String eventStreamName, String eventStreamVersion)
			throws AxisFault {
		int tenantId = PrivilegedCarbonContext.getThreadLocalCarbonContext().getTenantId();
		...
		if ((eventStreamName != null) && (!eventStreamName.isEmpty())) {
			if ((eventStreamVersion != null) && (!eventStreamVersion.isEmpty())) {
				...
				try {
					eventStreamService
							.removeEventStreamDefinition(eventStreamName, eventStreamVersion);
				} catch (EventStreamConfigurationException e) {
	...
	public EventStreamInfoDto[] getAllEventStreamDefinitionDto() throws AxisFault {
		...
		int tenantId = PrivilegedCarbonContext.getThreadLocalCarbonContext().getTenantId();
		try {
			Collection<EventStreamConfiguration> eventStreamDefinitionList =
					eventStreamService.getAllEventStreamConfigurations();
			if (eventStreamDefinitionList != null) {
				EventStreamInfoDto[] eventStreamInfoDtos =
						new EventStreamInfoDto[eventStreamDefinitionList.size()];
				int index = 0;
				for (EventStreamConfiguration eventStreamConfiguration : eventStreamDefinitionList) {
					...
					eventStreamInfoDtos[index].setStreamName(
							eventStreamConfiguration.getStreamDefinition().getName());
					eventStreamInfoDtos[index].setStreamVersion(
							eventStreamConfiguration.getStreamDefinition().getVersion());
					eventStreamInfoDtos[index].setStreamDefinition(
							eventStreamConfiguration.getStreamDefinition().toString());
					eventStreamInfoDtos[index].setStreamDescription(
							eventStreamConfiguration.getStreamDefinition().getDescription());
			...
			throw new AxisFault(
					"Error while retrieving event streams from store : " + e.getMessage(), e);
		}
	}
	...
	public String[] getStreamNames() throws AxisFault {
		...
		if (eventStreamService != null) {
			...
			try {
				...
				throw new AxisFault(
						"Error while retrieving stream names from store : " + e.getMessage(), e);
			}
		}
		return new String[0];
	}
	...
	public String getStreamDefinitionAsString(String streamId) throws AxisFault {
		...
		if (eventStreamService != null) {
			int tenantId = PrivilegedCarbonContext.getThreadLocalCarbonContext().getTenantId();
			EventStreamConfiguration streamDefinition =
					eventStreamService.getEventStreamConfiguration(streamId);
			...
			if (streamDefinition.getStreamDefinition().getMetaData() != null) {
				...
				for (Attribute attribute : streamDefinition.getStreamDefinition().getMetaData()) {
					...
					definitionString = definitionString + EventStreamConstants.META +
			...
			if (streamDefinition.getStreamDefinition().getCorrelationData() != null) {
				...
				for (Attribute attribute : streamDefinition.getStreamDefinition()
				                                           .getCorrelationData()) {
					if (appendComma) {
						definitionString = definitionString + ", ";
					}
					definitionString = definitionString + EventStreamConstants.CORRELATION +
					                   EventStreamConstants.ATTRIBUTE_SEPARATOR +
					                   attribute.getName() + " " +
					                   attribute.getType().name().toLowerCase();
					appendComma = true;
	...
	public EventStreamDefinitionDto getStreamDefinitionDto(String streamId) throws AxisFault {
		...
		if (eventStreamService != null) {
			...
			dto.setMetaData(convertAttributeList(
					eventStreamConfiguration.getStreamDefinition().getMetaData()));
			dto.setCorrelationData(convertAttributeList(
					eventStreamConfiguration.getStreamDefinition().getCorrelationData()));
			dto.setPayloadData(convertAttributeList(
					eventStreamConfiguration.getStreamDefinition().getPayloadData()));
	...
	public String convertEventStreamDefinitionDtoToString(
		...
		try {
			streamDefinition = new StreamDefinition(eventStreamDefinitionDto.getName(),
			                                        eventStreamDefinitionDto.getVersion());
		} catch (MalformedStreamDefinitionException e) {
		...
		streamDefinition.setDescription(eventStreamDefinitionDto.getDescription());
		streamDefinition.setNickName(eventStreamDefinitionDto.getNickName());
		streamDefinition.setCorrelationData(
				convertEventStreamAttributeDto(eventStreamDefinitionDto.getCorrelationData()));
		streamDefinition.setMetaData(
				convertEventStreamAttributeDto(eventStreamDefinitionDto.getMetaData()));
		streamDefinition.setPayloadData(
				convertEventStreamAttributeDto(eventStreamDefinitionDto.getPayloadData()));
	...
	public EventStreamDefinitionDto convertStringToEventStreamDefinitionDto(
		...
		eventStreamDefinitionDto
				.setCorrelationData(convertAttributeList(streamDefinition.getCorrelationData()));
		eventStreamDefinitionDto
				.setPayloadData(convertAttributeList(streamDefinition.getPayloadData()));

		eventStreamDefinitionDto
				.setStreamDefinitionString(new Gson().toJson(eventStreamDefinitionDto));
	...
	private EventStreamAttributeDto[] convertAttributeList(
		...
		if (attributeList != null) {
			EventStreamAttributeDto[] convertedAttributes =
					new EventStreamAttributeDto[attributeList.size()];
			int i = 0;
			for (org.wso2.carbon.databridge.commons.Attribute attribute : attributeList) {
				...
				convertedAttributes[i]
						.setAttributeType(attribute.getType().toString().toLowerCase());
				i++;
			}
	...
	private List<Attribute> convertEventStreamAttributeDto(
		...
		if (eventStreamAttributeDto != null) {
			for (int i = 0; i < eventStreamAttributeDto.length; i++) {
				...
				} else if (eventStreamAttributeDto[i].getAttributeType().equals("long")) {
					attributeType = AttributeType.LONG;
				} else if (eventStreamAttributeDto[i].getAttributeType().equals("double")) {
					attributeType = AttributeType.DOUBLE;
				} else if (eventStreamAttributeDto[i].getAttributeType().equals("float")) {
					attributeType = AttributeType.FLOAT;
				} else if (eventStreamAttributeDto[i].getAttributeType().equals("string")) {
					attributeType = AttributeType.STRING;
				} else if (eventStreamAttributeDto[i].getAttributeType().equals("boolean")) {
					attributeType = AttributeType.BOOL;
				}
				Attribute attribute =
						new Attribute(eventStreamAttributeDto[i].getAttributeName(), attributeType);
