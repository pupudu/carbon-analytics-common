...
public abstract class EventPublisher implements Runnable {
	...
	public EventPublisher(EventQueue<Event> eventQueue, GenericKeyedObjectPool transportPool,
	...
	public void run() {
		...
		while (true) {
			...
			if (event != null) {
				...
				eventBundle = convertToEventBundle(eventBundle, event,
	...
	private void publishEvent(Object eventBundle, ArrayList<Event> tempEventBundleHolder) {
		...
		try {
			...
			log.info("Session timed out for " + dataPublisherConfiguration.getPublisherKey() + "," +
			         e.getMessage());
	...
	private void notifyConnectionFailure(ArrayList<Event> tempEventBundleHolder) {
		if (null != receiverStateObserver) {
			...
			receiverStateObserver.notifyConnectionFailure(url.toString(), conf.getUserName(),
			                                              conf.getPassword());
			if (null != eventQueue) {
	...
	public String defineStream(String sessionId, String streamDefinition)
		...
		try {
			...
			throw new StreamDefinitionException(
					"Invalid type definition for stream " + streamDefinition, e);
		} catch (EventPublisherException e) {
			discardClient();
			...
			throw new DifferentStreamDefinitionAlreadyDefinedException(
					"Same stream with different definition already defined before sending this stream definitions to " +
					dataPublisherConfiguration.getPublisherKey(), e);
		} catch (MalformedStreamDefinitionException e) {
			returnClient(client);
			throw new MalformedStreamDefinitionException(
					"Malformed event definition :" + streamDefinition + " send  to " +
					dataPublisherConfiguration.getPublisherKey(), e);
		}
		return streamId;
	}

	private String redefineStream(Object client, String streamDefinition, String currentSessionId)
		...
		try {
			...
			returnClient(client);
			throw new DifferentStreamDefinitionAlreadyDefinedException(
					"Type already defined when send event definitions to" +
					dataPublisherConfiguration.getPublisherKey(), ex);
		} catch (StreamDefinitionException ex) {
			returnClient(client);
			throw new StreamDefinitionException(
					"Wrongly defined event definition after reconnection  :" + streamDefinition +
					" sent to " +
					dataPublisherConfiguration.getPublisherKey(), ex);
		} catch (MalformedStreamDefinitionException ex) {
			returnClient(client);
			throw new MalformedStreamDefinitionException(
					"Malformed event definition after reconnection  :" + streamDefinition +
					" sent to " +
					dataPublisherConfiguration.getPublisherKey(), ex);
		}
	}
	...
	public String findStreamId(String sessionId, String name, String version)
		...
		try {
			...
			throw new AgentException(
					"Error when finding event stream definition for : " + name + " " + version, e);
		}
		return streamId;
	...
	public boolean deleteStream(String sessionId, String streamId) throws AgentException {
		...
		try {
			...
			try {
				...
				discardClient();
				log.error("Cannot delete stream " + streamId + " at " +
	...
	public boolean deleteStream(String sessionId, String streamName, String streamVersion)
		...
		try {
			...
			try {
				...
				discardClient();
				log.error("Cannot delete stream " + streamName + " " + streamVersion + " at " +
				          dataPublisherConfiguration.getPublisherKey() +
				          " even after reconnecting ", ex);
				notifyConnectionFailure(null);
			} catch (SessionTimeoutException ex) {
			...
			notifyConnectionFailure(null);
			throw new AgentException(
					"Error when deleting event stream of: " + streamName + " " + streamVersion, e);
		}
		return status;
	}
		...
		if (reconnectionTime > 0) {
			try {
				dataPublisherConfiguration
						.setSessionId(agentAuthenticator.connect(dataPublisherConfiguration));
