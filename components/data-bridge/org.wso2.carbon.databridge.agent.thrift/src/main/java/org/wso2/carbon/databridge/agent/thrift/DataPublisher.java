...
public class DataPublisher {
	 ...
	 * @throws org.wso2.carbon.databridge.commons.exception.TransportException
	 */
	public DataPublisher(String receiverUrl, String userName, String password, Agent agent)
			throws MalformedURLException, AgentException, AuthenticationException,
			       TransportException {
		/**
		...
		if (receiverURL.isSecured()) {
			this.start(new ReceiverConfiguration(userName, password, receiverURL.getProtocol(),
			                                     ...
			                                     receiverURL.getProtocol(), (receiverURL.getHost()),
			                                     receiverURL.getPort(), receiverURL.isSecured()),
			...
			this.start(new ReceiverConfiguration(userName, password, receiverURL.getProtocol(),
			                                     ...
			                                     receiverURL.getProtocol(), (receiverURL.getHost()),
			                                     receiverURL.getPort() +
			                                     CommonThriftConstants.SECURE_EVENT_RECEIVER_PORT_OFFSET,
			                                     receiverURL.isSecured()), AgentHolder.getAgent());
		} else {
			throw new AgentException(
					"http not supported via this constructor use https, ssl or tcp ");
		}
	...
	public DataPublisher(String authenticationUrl, String receiverUrl, String userName,
		...
		this.start(new ReceiverConfiguration(userName, password, receiverURL.getProtocol(),
		                                     ...
		                                     authenticationURL.getHost(),
		                                     authenticationURL.getPort(), receiverURL.isSecured()),
		           AgentHolder.getAgent());
	...
	private void start(ReceiverConfiguration receiverConfiguration, Agent agent)
		...
		if (receiverConfiguration.isDataTransferSecured()) {
			this.eventPublisher = EventPublisherFactory
					.getEventPublisher(dataPublisherConfiguration, eventQueue, agent,
					                   agent.getSecureTransportPool());
		} else {
			this.eventPublisher = EventPublisherFactory
					.getEventPublisher(dataPublisherConfiguration, eventQueue, agent,
					                   agent.getTransportPool());
		}
		//Connect to the server
		dataPublisherConfiguration
				.setSessionId(agent.getAgentAuthenticator().connect(dataPublisherConfiguration));
	}
	 ...
	 * @return stream id
	 * @throws org.wso2.carbon.databridge.agent.thrift.exception.AgentException if client cannot publish the type definition
	 */
	@Deprecated public String findStream(String name, String version)
		...
		if (streamId == null) {
			throw new NoStreamDefinitionExistException(
					"Cannot find Stream Id for " + name + " " + version);
		}
		return streamId;
	}
	 ...
	 * @throws org.wso2.carbon.databridge.agent.thrift.exception.AgentException if client cannot publish the event
	 */
	public void publish(Event event) throws AgentException {
		try {
			AgentHolder.getAgent().getQueueSemaphore()
			           .acquire();//control the total number of buffered events
	...
	public boolean tryPublish(Event event) throws AgentException {
		try {
			if (AgentHolder.getAgent().getQueueSemaphore()
			               .tryAcquire()) {//control the total number of buffered events
				//Adds event to the queue and checks whether its scheduled for event dispatching
	...
	public void publish(String streamId, Object[] metaDataArray, Object[] correlationDataArray,
		...
		publish(new Event(streamId, System.currentTimeMillis(), metaDataArray, correlationDataArray,
		                  payloadDataArray));
	}
	...
	public boolean tryPublish(String streamId, Object[] metaDataArray,
		...
		return tryPublish(
				new Event(streamId, System.currentTimeMillis(), metaDataArray, correlationDataArray,
