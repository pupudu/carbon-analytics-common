...
public class ReceiverGroup implements ReceiverStateObserver {
	...
	public ReceiverGroup(ArrayList<DataPublisherHolder> properties) {
		...
		this.receiverGroupUnsentEventQueue = new LinkedBlockingQueue<PublishData>(
				AgentHolder.getOrCreateAgent().getAgentConfiguration()
				           .getLoadBalancingDataPublisherBufferedEventSize());
	}
	...
	protected void createDataPublishers(Agent agent,
		...
		scheduledExecutorService.scheduleAtFixedRate(new ReconnectionTask(), reconnectionInterval,
		                                             reconnectionInterval, TimeUnit.SECONDS);

	}

	protected void publish(String streamName, String streamVersion, long timeStamp,
		...
		if (null != dataPublisher) {
			dataPublisher.publish(streamName, streamVersion, timeStamp, metaDataArray,
			                      correlationDataArray, payloadDataArray, arbitraryDataMap);
		} else {
			receiverGroupUnsentEventQueue
	...
	protected void publish(String streamName, String streamVersion, Object[] metaDataArray,
	                       Object[] correlationDataArray, Object[] payloadDataArray,
	                       Map<String, String> arbitraryDataMap) throws AgentException {
		AsyncDataPublisher dataPublisher = getDataPublisher();
		if (null != dataPublisher) {
			dataPublisher.publish(streamName, streamVersion, metaDataArray, correlationDataArray,
			                      payloadDataArray, arbitraryDataMap);
		} else {
			receiverGroupUnsentEventQueue
					.offer(new PublishData(streamName, streamVersion, System.currentTimeMillis(),
					                       metaDataArray, correlationDataArray, payloadDataArray,
					                       arbitraryDataMap));
		}
	}
		...
		public void run() {
			...
			for (int i = START_INDEX; i <= maximumDataPublisherIndex; i++) {
				...
				if (!dataPublisherHolder.getConnected().get()) {
					...
					if (null != serverURL &&
					    !isServerExists(serverURL.getHost(), serverURL.getPort())) {
			...
			if (!isOneReceiverConnected) {
				if (isOneReceiverWasConnected.getAndSet(false)) {
					log.info(
							"No receiver is reachable at reconnection, will try to reconnect every " +
