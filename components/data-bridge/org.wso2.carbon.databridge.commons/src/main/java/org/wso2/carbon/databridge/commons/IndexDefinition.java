...
public class IndexDefinition {
	...
	public void setIndexData(String indexDefnStr, StreamDefinition streamDefinition) {
		...
		if (streamDefinition.getMetaData() != null) {
			for (Attribute attribute : streamDefinition.getMetaData()) {
				if (secIndexSet.contains(attribute.getName())) {
					secondaryIndexData
							.add(new Attribute("meta_" + attribute.getName(), attribute.getType()));
				}
				...
				if (fixedPropertiesList.contains(attribute.getName())) {
					tempFixProperties.put(attribute.getName(),
					                      new Attribute("meta_" + attribute.getName(),
					                                    attribute.getType()));
				}
			}
		}
		...
		if (streamDefinition.getCorrelationData() != null) {
			for (Attribute attribute : streamDefinition.getCorrelationData()) {
				if (secIndexSet.contains(attribute.getName())) {
					secondaryIndexData.add(new Attribute("correlation_" + attribute.getName(),
					                                     attribute.getType()));
				}
				if (custIndexSet.contains(attribute.getName())) {
					customIndexData.add(new Attribute("correlation_" + attribute.getName(),
					                                  attribute.getType()));
					custIndexSet.remove(attribute.getName());
				}
				if (fixedPropertiesList.contains(attribute.getName())) {
					tempFixProperties.put(attribute.getName(),
					                      new Attribute("correlation_" + attribute.getName(),
					                                    attribute.getType()));
				}
			}
		}
		...
		if (streamDefinition.getPayloadData() != null) {
			for (Attribute attribute : streamDefinition.getPayloadData()) {
				if (secIndexSet.contains(attribute.getName())) {
					secondaryIndexData.add(new Attribute("payload_" + attribute.getName(),
					                                     attribute.getType()));
				}
				if (custIndexSet.contains(attribute.getName())) {
					customIndexData.add(new Attribute("payload_" + attribute.getName(),
					                                  attribute.getType()));
					custIndexSet.remove(attribute.getName());
				}
				if (fixedPropertiesList.contains(attribute.getName())) {
					tempFixProperties.put(attribute.getName(),
					                      new Attribute("payload_" + attribute.getName(),
					                                    attribute.getType()));
				}
			}
		}
	...
	public void setIndexDataFromStore(String indexDefnStr) {
		...
		if (secIndexList != null) {
			...
			for (String secIndex : secIndexList) {
				...
				secondaryIndexData.add(new Attribute(secIndexData[0], AttributeType
						.valueOf(secIndexData[1].trim())));
			}
		}
		...
		if (custIndexList != null) {
			...
			for (String custIndex : custIndexList) {
				...
				if (name.startsWith("meta_")) {
					...
					metaCustomIndex.put(attributeName, new Attribute(attributeName, attributeType));
				} else if (name.startsWith("correlation_")) {
					if (correlationCustomIndex == null) {
						correlationCustomIndex = new HashMap<String, Attribute>();
					}
					correlationCustomIndex
							.put(attributeName, new Attribute(attributeName, attributeType));
					...
					generalCustomIndex.put(name, new Attribute(name, attributeType));
			...
			for (String fixedProperty : fixedSearchProps) {
				if (!fixedProperty.isEmpty()) {
					...
					AttributeType attributeType =
							AttributeType.valueOf(fixedPropertyData[1].trim());
		...
		if (arbitraryIndexList != null) {          //added later
			...
			for (String arbitraryIndexString : arbitraryIndexList) {
				...
				String name = custIndexData[0];
				AttributeType attributeType =
						AttributeType.valueOf(custIndexData[1].trim().split(",")[0]);
	...
	public String indexDefnString(List<Attribute> dataList) {
		...
		for (Attribute attribute : dataList) {
			...
			indexStringBuilder.append(attribute.getName()).append(":").append(attribute.getType());
			isRecordAdded = true;
		}
