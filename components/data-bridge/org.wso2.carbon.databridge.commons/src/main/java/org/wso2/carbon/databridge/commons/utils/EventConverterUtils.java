		...
		try {
			...
			for (int i = 0; i < eventObjects.length(); i++) {
				...
				if (event.getStreamId() == null || event.getStreamId().equals("")) {
					String errorMsg = "Stream Id cannot be null or empty, for JSON : " +
					                  eventObjects.get(i).toString();
			...
			String errorMsg = "Error converting JSON to event, for JSON : " + json;
			MalformedEventException malformedEventException =
					new MalformedEventException(errorMsg, e);
			if (log.isDebugEnabled()) {
				log.error(errorMsg, malformedEventException);
			} else {
				log.error(errorMsg);
				...
				for (Map.Entry<String, JsonElement> entry : entries) {
					...
					if (entry.getValue() instanceof JsonArray) {
						List<Attribute> attributeList =
								streamDefinition.getAttributeListForKey(key);
						...
						while (it.hasNext()) {
							Object value =
									getValue(jsonParser.parse(it.next().toString()).getAsString(),
