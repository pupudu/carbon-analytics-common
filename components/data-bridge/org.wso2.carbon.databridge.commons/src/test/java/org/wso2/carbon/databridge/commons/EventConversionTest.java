...
public class EventConversionTest extends TestCase {
	...
	@Test(expected = MalformedEventException.class) public void testNoStreamId() {
		try {
	...
	@Test(expected = MalformedEventException.class) public void testEmptyStreamId() {
		try {
	...
	@Test public void testEmptyEventArray() {
		List<Event> eventList = EventConverterUtils.convertFromJson(emptyArrayJSON);
		Event event = eventList.get(0);
		assertEquals(event.getPayloadData().length, 0);
	}

	@Test public void testRESTEventConversion() {
		List<Event> eventList =
				EventConverterUtils.convertFromJson(properJSON2, "foo::1.0.0-" + UUID.randomUUID());
		assertEquals(2, eventList.size());
		Event event = eventList.get(0);
		assertEquals(event.getCorrelationData().length, 1);
		assertEquals(event.getPayloadData().length, 2);
	...
	@Test(expected = MalformedEventException.class) public void testNullRESTEvents() {
		try {
	...
	@Test(expected = MalformedEventException.class) public void testEmptyRESTEvents() {
		try {
