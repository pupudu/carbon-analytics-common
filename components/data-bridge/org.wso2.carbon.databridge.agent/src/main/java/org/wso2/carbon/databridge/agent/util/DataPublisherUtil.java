...
public class DataPublisherUtil {
	...
	private static Object[] getEndpoints(String aURLGroup)
		...
		if (isLBURL && isFailOverURL) {
			throw new DataEndpointConfigurationException(
					"Invalid data endpoints URL set provided : " + aURLGroup +
			...
			for (int i = 0; i < receiverGroups.size(); i++) {
				...
				if (receiverGroup.length == authGroup.length) {
					...
					if (isFailOver != isAuthFailOver) {
						throw new DataEndpointConfigurationException(
								"Receiver and authentication URL group set " +
								"doesn't match. Receiver URL group: " + getURLSet(receiverGroup) +
								" is configured as failOver : " + isFailOver +
								", but Authentication URL group: " + getURLSet(authGroup) +
								" is configured as failOver :" + isAuthFailOver);
					}
				} else {
					throw new DataEndpointConfigurationException(
							"Receiver and authentication URL group set doesn't match. " +
							"Receiver URL group: " + getURLSet(receiverGroup) +
							", but Authentication URL group: " + getURLSet(authGroup));
				}
			}
		} else {
			throw new DataEndpointConfigurationException(
					"Receiver and authentication URL set doesn't match. " +
					"Receiver URL groups: " + receiverGroups.size() +
					", but Authentication URL groups: " + authGroups.size());
		}
	}
	 ...
	 */
	public static String getDefaultAuthURLSet(String receiverURLSet)
			throws DataEndpointConfigurationException {
