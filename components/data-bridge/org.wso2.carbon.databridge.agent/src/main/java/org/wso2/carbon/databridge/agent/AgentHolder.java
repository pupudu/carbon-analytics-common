...
public class AgentHolder {
	...
	private AgentHolder() throws DataEndpointAgentConfigurationException {
		try {
			...
			for (AgentConfiguration agentConfiguration : dataAgentsConfiguration
	...
	public synchronized static AgentHolder getInstance()
	...
	public synchronized DataEndpointAgent getDataEndpointAgent(String type)
		...
		if (agent == null) {
			throw new DataEndpointAgentConfigurationException(
					"No data agent configured for the type: " + type);
		}
		return agent;
	}
	...
	private DataAgentsConfiguration loadConfiguration()
			throws DataEndpointAgentConfigurationException {
		if (configPath == null)
			configPath = CarbonUtils.getCarbonConfigDirPath() +
		...
		try {
			...
			DataAgentsConfiguration dataAgentsConfiguration =
					(DataAgentsConfiguration) jaxbUnmarshaller.unmarshal(file);
			dataAgentsConfiguration.validateConfigurations();
			return dataAgentsConfiguration;
		} catch (JAXBException e) {
			throw new DataEndpointAgentConfigurationException(
					"Error while loading the configuration file " + configPath, e);
		}
	}
	 ...
	 * @throws DataEndpointAgentConfigurationException
	 */
	public DataEndpointAgent getDefaultDataEndpointAgent()
			throws DataEndpointAgentConfigurationException {
