...
public class BinaryDataEndpoint extends DataEndpoint {
	...
	@Override protected String login(Object client, String userName, String password)
		...
		try {
			return sendAndReceiveResponse(socket, BinaryEventConverter
					.createBinaryLoginMessage(userName, password), BinaryMessageConstants.LOGIN_OPERATION);
		} catch (Exception e) {
			if (e instanceof DataEndpointAuthenticationException) {
				...
				throw new DataEndpointAuthenticationException(
						"Error while trying to login to data receiver :" +
	...
	@Override protected void logout(Object client, String sessionId)
		...
		Socket socket = (Socket) client;
		try {
			sendAndReceiveResponse(socket,
			                       BinaryEventConverter.createBinaryLogoutMessage(sessionId),
			...
			if (e instanceof DataEndpointAuthenticationException) {
				...
				throw new DataEndpointAuthenticationException(
						"Error while trying to logout to data receiver :" +
						socket.getRemoteSocketAddress().toString(), e);
			}
		}
	}
	...
	@Override protected void send(Object client, List<Event> events)
		...
		try {
			sendAndReceiveResponse(socket, BinaryEventConverter
					.createBinaryPublishMessage(events, sessionId),
			...
			if (e instanceof DataEndpointException) {
				...
				throw new DataEndpointException(
						"Error while trying to publish events to data receiver :" +
						socket.getRemoteSocketAddress().toString(), e);
			}
		}
	}
	...
	private String sendAndReceiveResponse(Socket socket, String message, String operation)
			throws Exception {
			...
			if (responseLines[0].equals(BinaryMessageConstants.OK_RESPONSE)) {
				if (operation.equals(BinaryMessageConstants.LOGIN_OPERATION)) {
					if (responseLines.length == 3) {
						return responseLines[1]
								.replace(BinaryMessageConstants.SESSION_ID_PREFIX, "");
					} else {
						throw new DataEndpointAuthenticationException(
								"Unexpected response received from data receiver;" +
				...
				if (responseLines.length >= 5) {
					throw (Exception) (BinaryDataEndpoint.class.getClassLoader().
							loadClass(responseLines[1]).getConstructor(String.class)
					                                           .newInstance(responseLines[2]));
				} else {
					throw new DataEndpointException(
							"Unexpected error format received from receiver :" + response);
				}
			} else {
				throw new DataEndpointException(
						"Unexpected response received from data receiver : " + response);
			}
		} else {
			throw new DataEndpointException(
					"Unexpected empty response received from data receiver");
		}
	}
