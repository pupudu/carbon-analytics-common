...
public class BinaryDataReceiver {
	...
	public BinaryDataReceiver(BinaryDataReceiverConfiguration binaryDataReceiverConfiguration,
		...
		this.sslReceiverExecutorService =
				Executors.newFixedThreadPool(binaryDataReceiverConfiguration.
						                                                            getSizeOfSSLThreadPool());
		this.tcpReceiverExecutorService =
				Executors.newFixedThreadPool(binaryDataReceiverConfiguration.
	...
	private void startSecureTransmission() throws IOException, DataBridgeException {
		...
		log.info("Started Binary SSL Transport on port : " +
		         binaryDataReceiverConfiguration.getSSLPort());
	}
	...
	private void startEventTransmission() throws IOException {
		...
		log.info("Started Binary TCP Transport on port : " +
			...
			try {
				...
				while (true) {
					...
					RequestProcessor requestProcessor =
							new RequestProcessor(dataBridgeReceiverService);
					while ((messageLine = bufferedreader.readLine()) != null) {
						...
						try {
							...
							String errorMsg =
									"Error occurred while reading the message. " + e.getMessage();
							log.error(errorMsg, e);
							outputStream.write(BinaryMessageConverterUtil
									                   .getCompleteError(errorMsg, e));
							outputStream.flush();
							break;
						}
					}
				...
				log.error("Error while creating SSL socket on port : " +
				          binaryDataReceiverConfiguration.getSizeOfTCPThreadPool(), ex);
			}
		}
	}
}
