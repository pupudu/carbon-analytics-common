...
public class BinaryEventConverter implements EventConverter {
	...
	@Override public List<Event> toEventList(Object eventBundle,
	                                         StreamTypeHolder streamTypeHolder) {
		List<String> eventMessages =
				getSubMessage(BinaryMessageConstants.START_EVENT, BinaryMessageConstants.END_EVENT,
				              eventBundle.toString());
	...
	public Event getEvent(String eventMessage, StreamTypeHolder streamTypeHolder)
		...
		event.setTimeStamp(Long.parseLong(
				eventProps[1].replace(BinaryMessageConstants.TIME_STAMP_PREFIX, "")));
		...
		if (!metaData.isEmpty()) {
			if (metaData.size() > 1) {
				throw new MalformedEventException(
						"More than one meta data element found on event : " + eventMessage);
			}
			metaDataMessage = metaData.get(0);
		}
		List<String> correlationData = getSubMessage(BinaryMessageConstants.START_CORRELATION_DATA,
		                                             BinaryMessageConstants.END_CORRELATION_DATA,
		                                             eventMessage);
		if (!correlationData.isEmpty()) {
			if (correlationData.size() > 1) {
				throw new MalformedEventException(
						"More than one correlation data element found on event : " + eventMessage);
			}
		...
		List<String> payloadData = getSubMessage(BinaryMessageConstants.START_PAYLOAD_DATA,
		                                         BinaryMessageConstants.END_PAYLOAD_DATA,
		                                         eventMessage);
		if (!payloadData.isEmpty()) {
			if (payloadData.size() > 1) {
				throw new MalformedEventException(
						"More than one meta payload element found on event : " + eventMessage);
			}
			payloadDataMessage = payloadData.get(0);
		}
		List<String> arbitraryData = getSubMessage(BinaryMessageConstants.START_ARBITRARY_DATA,
		                                           BinaryMessageConstants.END_ARBITRARY_DATA,
		                                           eventMessage);
		if (!arbitraryData.isEmpty()) {
			if (arbitraryData.size() > 1) {
				throw new MalformedEventException(
						"More than one meta arbitrary element found on event : " + eventMessage);
			}
		...
		if (attributeTypeOrder == null) {
			...
			if (attributeTypeOrder == null) {
				throw new EventConversionException(
						"No StreamDefinition for streamId " + event.getStreamId() +
						" present in cache ");
			}
		}
	...
	public Object[] toObjectArray(String attributeMessage, AttributeType[] attributeTypeOrder,
	                              String type) {
		if (attributeTypeOrder != null) {
	...
	public Map<String, String> toStringMap(String attributeMessage) {
		if (attributeMessage != null) {
			...
			for (String aAttribute : attributes) {
				String[] aProperty =
						aAttribute.trim().split(BinaryMessageConstants.PARAMS_SEPARATOR);
				if (aProperty.length != 2) {
					throw new MalformedEventException(
							"Arbitrary data sent in the events are not in " +
