...
public final class ThriftEventConverter implements EventConverter {
	...
	public Object[] toObjectArray(ThriftEventBundle thriftEventBundle,
	                              AttributeType[] attributeTypeOrder, IndexCounter indexCounter) {
		if (attributeTypeOrder != null) {
			...
			for (int i = 0; i < attributeTypeOrder.length; i++) {
				switch (attributeTypeOrder[i]) {
					case INT:
						objects[i] = thriftEventBundle.getIntAttributeList()
						                              .get(indexCounter.getIntCount());
						indexCounter.incrementIntCount();
						break;
					case LONG:
						objects[i] = thriftEventBundle.getLongAttributeList()
						                              .get(indexCounter.getLongCount());
					...
					case FLOAT:
						...
						break;
					case BOOL:
						objects[i] = thriftEventBundle.getBoolAttributeList()
						                              .get(indexCounter.getBoolCount());
	...
	public List<Event> toEventList(Object eventBundle, StreamTypeHolder streamTypeHolder) {
		if (eventBundle instanceof ThriftEventBundle) {
			...
			throw new EventConversionException(
					"Wrong type event relieved " + eventBundle.getClass());
		}
	...
	private List<Event> createEventList(ThriftEventBundle thriftEventBundle,
		...
		try {
			for (int i = 0; i < thriftEventBundle.getEventNum(); i++) {
				...
				streamId = thriftEventBundle.getStringAttributeList()
				                            .get(indexCounter.getStringCount());
				indexCounter.incrementStringCount();
				event.setStreamId(streamId);
				long timeStamp =
						thriftEventBundle.getLongAttributeList().get(indexCounter.getLongCount());
				indexCounter.incrementLongCount();
				...
				if (attributeTypeOrder == null) {
					...
					if (attributeTypeOrder == null) {
						throw new EventConversionException(
								"No StreamDefinition for streamId " + streamId +
								" present in cache ");
					}
				}
				event.setMetaData(
				...
				if (thriftEventBundle.isSetArbitraryDataMapMap()) {
					Map<String, String> arbitraryData =
							thriftEventBundle.getArbitraryDataMapMap().get(i);
			...
			throw new EventConversionException(
					"Error when converting " + streamId + " of event bundle with events " +
