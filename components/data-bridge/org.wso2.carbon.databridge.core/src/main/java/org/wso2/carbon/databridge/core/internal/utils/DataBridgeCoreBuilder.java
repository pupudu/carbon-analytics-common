...
public final class DataBridgeCoreBuilder {
	...
	public static List<String[]> loadStreamDefinitionXML() throws DataBridgeConfigurationException {
		...
		if (file.exists() && !file.isDirectory()) {
			...
			if (config != null) {
				if (!(new QName(DataBridgeConstants.DATA_BRIDGE_NAMESPACE,
				                DataBridgeConstants.STREAM_DEFINITIONS_ELEMENT))
						.equals(config.getQName())) {
					throw new DataBridgeConfigurationException("Wrong configuration added in " +
					                                           DataBridgeConstants.STREAM_DEFINITIONS_XML);
				}
				for (Iterator streamDefinitionIterator =
						     config.getChildElements(); streamDefinitionIterator.hasNext(); ) {
					OMElement streamDefinition = (OMElement) streamDefinitionIterator.next();
					String domainName = streamDefinition.getAttributeValue(
							new QName(DataBridgeConstants.DOMAIN_NAME_ATTRIBUTE));

					if (domainName == null || domainName.equals("")) {
						domainName = MultitenantConstants.SUPER_TENANT_DOMAIN_NAME;
					}
					streamDefinitionList
							.add(new String[] { domainName, streamDefinition.getText() });
				}
			}
		}
	...
	public static String getDatabridgeConfigPath() {
		String carbonHome = System.getProperty(ServerConstants.CARBON_CONFIG_DIR_PATH);
		return carbonHome + File.separator + DataBridgeConstants.DATA_BRIDGE_DIR + File.separator +
		       DataBridgeConstants.DATA_BRIDGE_CONFIG_XML;
	}

	public static OMElement loadXML(String path, String fileName)
