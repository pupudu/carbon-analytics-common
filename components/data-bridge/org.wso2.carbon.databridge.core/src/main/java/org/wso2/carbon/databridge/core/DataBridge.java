...
public class DataBridge implements DataBridgeSubscriberService, DataBridgeReceiverService {
	...
	public DataBridge(AuthenticationHandler authenticationHandler,
		...
		try {
			...
			DataBridgeConfiguration dataBridgeConfiguration =
					(DataBridgeConfiguration) jaxbUnmarshaller.unmarshal(file);
			this.eventDispatcher =
					new EventDispatcher(streamDefinitionStore, dataBridgeConfiguration,
					                    authenticationHandler);
			this.streamDefinitionStore = streamDefinitionStore;
			authenticatorHandler = authenticationHandler;
			authenticator = new Authenticator(authenticationHandler, dataBridgeConfiguration);
		} catch (JAXBException e) {
			log.error("Error while loading the data bridge configuration file : " +
			          dataBridgeConfigPath, e);
		}
	}
	...
	public String defineStream(String sessionId, String streamDefinition, String indexDefinition)
			...
			DifferentStreamDefinitionAlreadyDefinedException, MalformedStreamDefinitionException,
			SessionTimeoutException {
		AgentSession agentSession = authenticator.getSession(sessionId);
		if (agentSession.getCredentials() == null) {
			if (log.isDebugEnabled()) {
		...
		try {
			authenticatorHandler.initContext(agentSession);
			String streamId =
					eventDispatcher.defineStream(streamDefinition, agentSession, indexDefinition);
			if (streamId != null) {
				int tenantId = agentSession.getCredentials().getTenantId();
				for (StreamAddRemoveListener streamAddRemoveListener : streamAddRemoveListenerList) {
					streamAddRemoveListener.streamAdded(tenantId, streamId);
	...
	public String findStreamId(String sessionId, String streamName, String streamVersion)
		...
		try {
			authenticatorHandler.initContext(agentSession);
			return eventDispatcher.findStreamId(streamName, streamVersion, agentSession);
		} catch (StreamDefinitionStoreException e) {
	...
	public boolean deleteStream(String sessionId, String streamId) throws SessionTimeoutException {
		...
		try {
			...
			status = eventDispatcher
					.deleteStream(DataBridgeCommonsUtils.getStreamNameFromStreamId(streamId),
					              DataBridgeCommonsUtils.getStreamVersionFromStreamId(streamId),
					              agentSession);
			if (status) {
				for (StreamAddRemoveListener streamAddRemoveListener : streamAddRemoveListenerList) {
					streamAddRemoveListener
							.streamRemoved(agentSession.getCredentials().getTenantId(), streamId);
				}
			}
		} finally {
			authenticatorHandler.destroyContext(agentSession);
	...
	@Override public StreamDefinition getStreamDefinition(String sessionId, String streamName,
		...
		try {
			...
			return getStreamDefinition(streamName, streamVersion,
			                           agentSession.getCredentials().getTenantId());
		} finally {
			authenticatorHandler.destroyContext(agentSession);
	...
	@Override public List<StreamDefinition> getAllStreamDefinitions(String sessionId)
			throws SessionTimeoutException {
		AgentSession agentSession = authenticator.getSession(sessionId);
		if (agentSession.getUsername() == null) {
			if (log.isDebugEnabled()) {
	...
	@Override public void saveStreamDefinition(String sessionId, StreamDefinition streamDefinition)
	...
	@Override public StreamDefinition getStreamDefinition(String streamName, String streamVersion,
	                                                      int tenantId)
	...
	@Override public StreamDefinition getStreamDefinition(String streamId, int tenantId)
			throws StreamDefinitionNotFoundException, StreamDefinitionStoreException {
		return streamDefinitionStore.getStreamDefinition(streamId, tenantId);

	}

	@Override public List<StreamDefinition> getAllStreamDefinitions(int tenantId) {
		return new ArrayList<StreamDefinition>(
				streamDefinitionStore.getAllStreamDefinitions(tenantId));
	}

	@Override public void saveStreamDefinition(StreamDefinition streamDefinition, int tenantId)
			throws DifferentStreamDefinitionAlreadyDefinedException,
			       StreamDefinitionStoreException {
		streamDefinitionStore.saveStreamDefinition(streamDefinition, tenantId);
	}

	@Override public boolean deleteStreamDefinition(String streamName, String streamVersion,
	                                                int tenantId) {
